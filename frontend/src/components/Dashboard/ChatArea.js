...
export const ChatArea = () => {
  ...
  useEffect(() => {
    socket.on("typing", ({ typer }) => {
      if (typer !== user._id) setIsOtherUserTyping(true);
    });

    socket.on("stop-typing", ({ typer }) => {
      if (typer !== user._id) setIsOtherUserTyping(false);
    });
  ...
  const handleSendMessage = async (e, messageText = "", file = null) => {
    e.preventDefault();
    ...
    setMyChatList(
      myChatList
        .map((c) =>
          c._id === activeChatId
            ? { ...c, latestmessage: messageText, updatedAt: new Date() }
            : c
        )
        .sort((a, b) => new Date(b.updatedAt) - new Date(a.updatedAt))
    );
